...
kubectl get node k8s0 -o jsonpath='{.status.allocatable.gpu\.intel\.com/i915_monitoring}{"\n"}'
```
...
DaemonSet. The exporter uses the plugin's `gpu.intel.com/i915_monitoring` resource so
